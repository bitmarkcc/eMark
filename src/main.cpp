...
const string strMessageMagic = "eMark Signed Message:\n";
...
static int64_t MAX_MINT_PROOF_OF_STAKE_1 = .038 * 50 * COIN;	//190% annual stake
static int64_t MAX_MINT_PROOF_OF_STAKE_2 = .038 * COIN;	//3.8% annual stake
...
{
	int64_t MAX_MINT_PROOF_OF_STAKE = (nTime > nHardforkTime) ? MAX_MINT_PROOF_OF_STAKE_2 : MAX_MINT_PROOF_OF_STAKE_1;

    int64_t nSubsidy;
    if (nTime > nHardforkTime)
        nSubsidy = (nCoinAge * 33 * MAX_MINT_PROOF_OF_STAKE) / (365 * 33 + 8);
    else
        nSubsidy = nCoinAge * 33 / (365 * 33 + 8) * MAX_MINT_PROOF_OF_STAKE;
...
{
    if (!TestNet() && pindexLast->nTime < nHardforkTime2)
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    if (IsProofOfStake())
    {
        ...
        int64_t nCalculatedStakeReward = GetProofOfStakeReward(nCoinAge, nBits, nTime);
...
bool CBlock::SetBestChain(CTxDB& txdb, CBlockIndex* pindexNew)
{
    ...
    if (!fIsInitialDownload)
    {
       ...
       // As long as  miners may change nVersion just to meet difficulty target, following warning makes no sense
       //if (nUpgraded > 100/2)
...
bool CBlock::AcceptBlock()
{
    ...
    if (IsProtocolV3(nTime) && nVersion < 7)
        return DoS(100, error("AcceptBlock() : reject too old nVersion = %d", nVersion));
    else if (!IsProtocolV3(nTime) && nVersion > 4)
    ...
    if (nBits != GetNextTargetRequired(pindexPrev, IsProofOfStake()))
        return DoS(100, error("AcceptBlock() : incorrect %s, nBits: %d, Target: %d", IsProofOfWork() ? "proof-of-work" : "proof-of-stake", nBits, GetNextTargetRequired(pindexPrev, IsProofOfStake())));
...
bool CBlock::SignBlock(CWallet& wallet, int64_t nFees)
{
    ...
    {
        int64_t nSearchInterval = IsProtocolV3(nTime) ? 1 : nSearchTime - nLastCoinStakeSearchTime;
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version")
    {
        ...
        if (pfrom->nVersion < 60007 && GetAdjustedTime() > nHardforkTime)
            badVersion = true;
        if (pfrom->nVersion < 60010 && GetAdjustedTime() > nHardforkTime2)
            badVersion = true;
